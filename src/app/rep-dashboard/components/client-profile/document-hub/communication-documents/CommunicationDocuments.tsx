...
interface CommunicationDocumentsProps {
  searchTerm: string;
  onUploadDocument: (docType: string) => void;
}

const CommunicationDocuments: React.FC<CommunicationDocumentsProps> = ({ 
  searchTerm, 
  onUploadDocument 
}) => {
  const category = documentCategories.communication;
  const categoryDocuments = mockDocuments.filter(doc => doc.category === 'communication');
...
export default CommunicationDocuments;
