...
export interface UseLeadIntakeReturn {
  processSpreadsheet: (data: any[], campaignId: string) => Promise<Lead[]>;
...
export function useLeadIntake(): UseLeadIntakeReturn {
  ...
  const processSpreadsheet = useCallback(async (data: any[], campaignId: string): Promise<Lead[]> => {
    setIsProcessing(true);
    setError(null);

    try {
      const leads = await leadIntakeService.processSpreadsheetData(data, campaignId);
      return leads;
    } catch (err) {
