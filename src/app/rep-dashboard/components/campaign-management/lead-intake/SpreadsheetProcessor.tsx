...
export default function SpreadsheetProcessor({
  ...
  const processFile = useCallback(async (file: File) => {
    ...
    try {
      ...
      // Convert to objects and add campaign info
      const processedData = rows.map((row, index) => {
        const leadData: any = { 
          rowIndex: index + 2, // +2 because we skip header and 0-index
          campaignId: campaignId // Add campaign ID to each lead
        };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 rounded-lg p-4 border border-blue-200">
        <h3 className="text-sm font-semibold text-blue-900 mb-1">📋 Target Campaign</h3>
        <p className="text-blue-800">Leads will be added to: <strong>{campaignId}</strong></p>
      </div>
          ...
          {uploadStatus === 'processing' ? (
            ...
            <>
              ...
              <p className="text-sm text-gray-600">
                {previewData.length} leads detected for {campaignId}. Click "Validate Data" to continue.
              </p>
            </>
          ) : uploadStatus === 'error' ? (
            <>
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-100">
                <tr>
                  {Object.keys(previewData[0] || {}).filter(key => !['rowIndex', 'campaignId'].includes(key)).slice(0, 6).map((header) => (
                  ...
                  {Object.keys(previewData[0] || {}).length > 8 && (
                    <th className="px-3 py-2 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                      ... +{Object.keys(previewData[0] || {}).length - 8} more columns
                    </th>
                  )}
                </tr>
              </thead>
              <tbody className="bg-white divide-y divide-gray-200">
                {previewData.map((row, index) => (
                  <tr key={index}>
                    {Object.entries(row).filter(([key]) => !['rowIndex', 'campaignId'].includes(key)).slice(0, 6).map(([key, value]) => (
