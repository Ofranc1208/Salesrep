...
/**
 * DataMapper - Handles row-to-Lead object mapping and data transformation
 * Focused responsibility: Transform spreadsheet rows into structured Lead objects
 */
export class DataMapper {
  /**
   * Map a spreadsheet row to a Lead object
   */
  mapRowToLead(row: any, campaignId: string): Lead {
    // Extract phone numbers from various possible columns
    const phoneNumbers = this.extractPhoneNumbers(row);

    // Generate unique ID
    const leadId = this.generateLeadId(row, campaignId);
    ...
    const lead: Lead = {
      ...
      campaignName: campaignId, // Use the provided campaign ID
      phoneNumbers,
  ...
  private extractAdditionalFields(row: any): Record<string, any> {
    const standardFields = new Set([
      ...
      'Notes', 'notes', 'rowIndex', 'campaignId'
    ]);
   ...
   */
  private generateLeadId(row: any, campaignId: string): string {
    const timestamp = Date.now();
    const clientName = this.extractClientName(row).replace(/\s+/g, '');
    const random = Math.random().toString(36).substring(2, 8);
    const campaignPrefix = campaignId.replace(/\s+/g, '').substring(0, 8).toUpperCase();
    return `${campaignPrefix}_${clientName.substring(0, 8).toUpperCase()}_${timestamp}_${random}`;
  }
}
